<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:ff758c,100:ff7eb3&height=250&section=header&text=Sara%20Ferreira%20Rosmaninho%20Roza&fontSize=38&fontColor=ffffff&animation=fadeIn&desc=QA%20Engineer&descAlignY=70&descSize=18"/>
</p>

<p align="center">
  <img src="https://cdn-icons-png.flaticon.com/512/1055/1055687.png" width="120"/>
</p>

<h1 align="center">🧪 Quality Assurance</h1>

<p align="center">
💼 4 anos em QA <br>
🏦 10 anos no setor bancário <br>
🎓 ADS - TechMob 4.0 <br>
🔍 Teste de Performace • API • End-to-End • Teste de Estresse
</p>

---

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?color=ff7eb3&size=22&center=true&vCenter=true&width=520&lines=Especialista+em+Qualidade+de+Software;Testes+em+Sistemas+Bancários;API+Testing+%7C+E2E+Automation;Foco+em+Segurança+e+Confiabilidade"/>
</p>

---

## 👩‍💻 Sobre mim

Profissional com sólida experiência no setor bancário, atuando diretamente na validação de sistemas críticos e garantindo a integridade de dados e regras de negócio.

Com *4 anos de experiência em QA* e *10 anos no setor financeiro*, possuo forte domínio em testes **Teste de Performace, API e Teste de Estresse** e automação de fluxos *end-to-end*.

Atualmente cursando *Análise e Desenvolvimento de Sistemas pela TechMob 4.0*, ampliando conhecimentos técnicos para atuação ainda mais estratégica na área de qualidade.

---

## ⚙️ Tecnologias

<p align="center">
<img src="https://img.shields.io/badge/Cypress-Automation-brightgreen"/>
<img src="https://img.shields.io/badge/Selenium-Testing-green"/>
<img src="https://img.shields.io/badge/Postman-API-orange"/>
<img src="https://img.shields.io/badge/RestAssured-Java-red"/>
<img src="https://img.shields.io/badge/Java-Backend-red"/>
<img src="https://img.shields.io/badge/JavaScript-Tests-yellow"/>
<img src="https://img.shields.io/badge/SQL-Database-blue"/>
<img src="https://img.shields.io/badge/Docker-Containers-blue"/>
<img src="https://img.shields.io/badge/Git-VersionControl-orange"/>
<img src="https://img.shields.io/badge/GitHub-Repos-black"/>
<img src="https://img.shields.io/badge/Jira-Management-blue"/>
<img src="https://img.shields.io/badge/Zephyr-Scale-purple"/>
<img src="https://img.shields.io/badge/AzureDevOps-Testing-blue"/>
<img src="https://img.shields.io/badge/BDD-Gherkin-green"/>
</p>

---

## 💼 Experiência

Atuação no setor bancário com foco em qualidade de software, participando de validações de sistemas financeiros, testes de integrações e análise de impacto em operações críticas.

### 🔧 Atividades e Responsabilidades

* BDD com Gherkin
* Testes regressivos
* Acompanhamento de deploy em produção
* Auxílio na definição das regras dos projetos junto ao PO
* Participação em refinamentos técnicos com Arquiteto e PO
* Projeção de entregas
* Utilização do Jira com plugin Zephyr Scale
* Utilização do Azure DevOps (Test Case e Test Plan)
* Apoio aos desenvolvedores no entendimento das regras de negócio
* Disseminação de conhecimento das regras
* Treinamento de novos QAs
* Compartilhamento de novas ferramentas implementadas
* Apoio na criação de ferramentas de testes

---

## 🎯 Objetivo

Garantir a qualidade de sistemas através de testes eficientes, visão analítica e foco no impacto real para o negócio, evoluindo continuamente em automação e qualidade de software.

---

<p align="center">
✨ <b>"Qualidade não é apenas encontrar erros, é garantir confiança."</b> ✨
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:ff7eb3,100:ff758c&height=120&section=footer"/>
</p>
